...
wiktionary_defined_headword_query = select([
    headword.c.name,
    headword.c.redlink,
    case(
        value=func.sum(case(
            value=word_sense.c.sense,
            whens={"": 0},
            else_=1
        )),
        whens={0: False},
...
wiktionary_deriv_query = select([
    ...
    headword.join(
        derived_term,
        derived_term.c.headword_id == headword.c.id
    )
...
def headword_id_query(lemma_query):
    return select([
        tables["headword_freq"].c.id,
    ]).select_from(
        tables["headword_freq"],
    ).where(
        tables["headword_freq"].c.lemma_query == lemma_query,
    )
    ...
    return select(
        ...
        reduce(
            lambda acc, elm: acc.outerjoin(
                tables[elm],
                tables[elm].c.mwe_id == tables["ud_mwe"].c.id
            ),
            RELATED_TABLES,
            # , "propbank_surv"
            tables["ud_mwe"],
...
def headword_grouped(headwords=None, typ=None):
    query = select([
        ...
        tables["ud_mwe"].join(
            ...
            (tables["ud_mwe_token"].c.subword_idx == tables["ud_mwe"].c.headword_idx)
        ).join(
            tables["mwe_fmt"],
            tables["mwe_fmt"].c.mwe_id == tables["ud_mwe"].c.id
        ).join(
            tables["link"],
