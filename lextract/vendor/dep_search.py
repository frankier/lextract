...
class DepSearch:
    ...
    def query(self, search):
        ...
        (
            ...
            solr_args,
            solr_or_groups,
        ) = query_obj.map_set_id(self.set_id_db)
        fdb = self.fdb_class.Query(
            [],
            [item[1:] for item in solr_args if item.startswith("!")],
            solr_or_groups,
            self.db_args["dir"],
            self.args.case,
            query_obj,
        ...
        yield from query_from_db(
            query_obj, self.args, self.db, fdb, self.set_id_db, self.comp_dict
        )
